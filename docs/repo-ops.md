...
- Release: `git push origin dev:main`, once the head's `validate` run is green and the human has emptied, or accepted item by item, what the landed PRs list as not measured. CI runs on push heads only, so the releasable commits are exactly the SHAs that were once a pushed `dev` head; with a red head, either release the last green SHA (`git push origin <sha>:main`) or land a fix commit and release that. The rulesets guarantee this is the only kind of push `main` accepts.

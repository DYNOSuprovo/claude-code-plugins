...
Layers, dependency direction and the rules of the code, imported here so they load with
this file:

@.claude/rules/architecture.md

The drawings, the assessment and the shape chosen for the next phases, for the people who
change the tree: `docs/architecture.md`.
...
- Every value crossing HTTP or a plugin boundary is JSON: `src/protocol.ts` is the one place it is typed; the domain types it carries are re-exported from `src/domain/`.
- `src/boundaries.test.ts` holds the dependency direction; a new import that fails it is in the wrong layer, not a test to loosen.

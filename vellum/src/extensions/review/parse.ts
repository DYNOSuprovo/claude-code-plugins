...
const LINES = /^lines? (\d+)(?:[–-](\d+))?: (.+)$/u;

type List = (typeof LISTS)[keyof typeof LISTS];

type Item = { readonly section: string | null; readonly rest: string; readonly quote: string[] };
...
function findingOf({ section, rest, quote }: Item): Finding {
  ...
  if (quote.length === 0 || text === undefined || first < 1 || last < first) {
    ...
    return { section, text: `${rest}${quoted}`, place: null };
  }

  const place: Place = { lines: [first, last], quote: quote.join("\n") };

  return { section, text, place };
}
...
export function parseVerdict(text: string): Verdict | null {
  ...
  for (const line of lines.slice(start + 1).map((raw) => raw.trimEnd())) {
    ...
    if (isStatus(line) && status === null) status = STATUS[line];
      ...
      items[list].push({ section: item[1] ?? null, rest: item[2] ?? "", quote: [] });
    } else return null;
  }

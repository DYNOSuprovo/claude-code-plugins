...
4. Comments: the **Comment** switch, off at every load, starts them, and the key `C` flips it. On, a drag in a Markdown document quotes a passage and a click picks a block, a link, a code block, a table cell or a diagram; Ctrl adds either one to the same comment. In an HTML mockup a click picks an element, and Ctrl+click adds another. Off, the page selects, copies and follows links as any page does, and a mockup runs its own scripts. The same popover takes a quick label (Clarify, Verify, Too much, Missing check), with a detail or without, and **Delete this**; the feedback prints each as a sentence Claude acts on. The box under the comments takes a general comment. Code blocks are coloured and Mermaid blocks are drawn.
